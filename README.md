...
Tabulation System for Bb. Alinsangan 
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/bb-alinsangan`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `bb-alinsangan` and import [bb-alinsangan.sql](bb-alinsangan.sql) into it.

6. Open the terminal and navigate to the project directory **bb-alinsangan**.
...
9. Open your web browser and access <http://localhost:5206/bb-alinsangan/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-bb-alinsangan` and import [bb-alinsangan.sql](bb-alinsangan.sql) into it.
...
2. Access the application by visiting `http://[host_name]/bb-alinsangan`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/bb-alinsangan>
     - <http://192.168.1.99/bb-alinsangan>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/bb-alinsangan/app>.
